...
export default function CompleteApplication() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!loggedIn) {
        ...
        navigate('/login?redirect=/admission/complete-application');
        return;
      }
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-[#f6f8fc]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-cod-blue mx-auto"></div>
  ...
  async function handleSubmit(e) {
    ...
    try {
      // Prepare application data for the backend
      const applicationData = {
        programmeId: "prog-1", // You should get this from the course selection
      ...
      navigate("/payment", { 
        state: { 
          applicationId: application.admission?.id || application.id, 
          formData: form, 
          trackName, 
          course 
        } 
